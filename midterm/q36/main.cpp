...
//Probably on midterm ****
//36 on midterm
//wants us to add special char to required
...
void displayRequirements()
{
    ...
    cout << "The Password should contain at least one Special Character (ie. ~`!@#$%^&*()-_+={}[];:<>,./?)!" << endl;
}
...
bool isValid(char str[])
{
    ...
    if (hasSpecial(str) && hasLength(str) && hasLower(str) && hasUpper(str) && hasDigit(str)){
        status = true;
    }
...
void displayResults(char str[])
{
    ...
    if(isValid(str)){
        ...
        //why invalid?
        if(!hasSpecial(str)){
            cout << "Password should have at least ONE special Character!\n";
        }
